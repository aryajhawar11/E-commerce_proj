...
const List = ({token}) => {
    ...
    const fetchList = async () => {
        try {                  
            const response = await axios.get(backendUrl + '/api/product/list'); //API endpoint
        ...
        {/* Product List */}
        {
            list.map((item, index) => (
                <div key={index} className='grid grid-cols-[1fr_3fr_1fr_1fr_1fr] items-center py-1 px-2  text-sm'>
                    <img className='w-20 object-cover' src={item.image[0]} alt={item.name} />
                    <p className='text-sm'>{item.name}</p>
                    <p className='text-sm'>{currency}{item.price}</p>
                    <p className='text-sm'>{item.category}</p>
                    <div className='flex gap-2 justify-center'>
                        <button onClick={()=>removeProduct(item._id)} className='p-5 cursor-pointer' >X</button>
                    </div>
                </div>
            ))
        }
